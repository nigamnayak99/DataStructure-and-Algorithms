...
public class BuildLPSNaive {
	...
	private static int longestPrefixSuffix(String pattern, int n) {
		// For i = 0, lps[0]  always .
		// We are finding lps value till the current position in a string .
		//So it can't be greater or equal to current position . 
		//It will always be less than current position . e.g for position i = 4 , it can't be greater than or equal to 4 ,it will be always lps[i] < i.
		// for position 4 , lps[i] can be 3,2,1,0 . So len represents possible length of lps , and we are traversing in decreasing order .
	...
	//We are going to fill corresponding lps string length,
